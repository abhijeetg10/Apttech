  ...
  return (
    <aside className={styles.sidebar}>
      ...
      <nav className={styles.nav}>
        {links.map((link) => {
          const isActive = pathname === link.path || (pathname.startsWith(link.path) && link.path !== "/student" && link.path !== "/admin");
          return (
            <Link 
              key={link.path} 
              href={link.path}
      ...
      <div className={styles.footer}>
        <div className={styles.navLink} style={{ cursor: "pointer", marginTop: "auto" }} onClick={() => signOut({ callbackUrl: "/" })}>
